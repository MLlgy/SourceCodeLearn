 ...
 */
package com.example.mkio.importsource.okio;
...
final class Segment {
  ...
  // 游标：下一个要读取到的字节在数组中的位置
  int pos;

  /** The first byte of available data ready to be written to. */
  // 游标：剩下缓冲区可以写入的第一个字节在数组中位置
  int limit;

  /** True if other segments or byte strings use the same byte array. */
  // r如果其他的 segment 或者字节串使用该 Segment 的字节数组，那么需要返回 true
  boolean shared;

  /** True if this segment owns the byte array and can append to it, extending {@code limit}. */
  //如果该 Segmentg 拥有该字节数组的所有权，并且可以追加数据，返回为 true
  boolean owner;
   ...
   *
   * 返回一个可以与该 Segment 共享字节数组的 Segment，调整 pos 和 limit 是合法的，但是写入数据时非法的，这个操作会使当前的 Segment 变为 shared ，并会使该 Segment 不被
   */
